...
def create_multimodal_optimizer_bnb(args: 'TrainingArguments', model, dataset):
    """ViT/Aligner/LLM use different learning rates."""
    from swift.llm import get_model_arch
    decay_parameters = set(Trainer.get_decay_parameter_names(None, model))
    model_arch = get_model_arch(model.model_meta.model_arch)
    ...
    optimizer_cls, optimizer_kwargs = Trainer.get_optimizer_cls_and_kwargs(args, model)
    return bnb.optim.Adam8bit(optimizer_grouped_parameters, **optimizer_kwargs, min_8bit_size=16384, eps=1e-5), None
...
optimizers_map = {
    ...
    'custom_multimodal': create_multimodal_optimizer_bnb
}
